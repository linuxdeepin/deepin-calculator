...
TextButtonTab::TextButtonTab(const QString &text, bool page, QWidget *parent)
    : DPushButton(text, parent)
    , m_effect(new QGraphicsDropShadowEffect(this))
{
    setFocusPolicy(Qt::TabFocus);
    setObjectName("TextButtonTab");
...
TextButtonTab::~TextButtonTab()
{
    // delete m_effect;
}

void TextButtonTab::init()
{
    //按钮初始化时确认当前是否是上标按钮
    if (text() == "2ⁿᵈ") {
        m_btn = 1;
 ...
 * @param msec 100ms
 */
void TextButtonTab::animate(bool isspace, int msec)
{
 ...
 * @brief 鼠标按下
 */
void TextButtonTab::mousePressEvent(QMouseEvent *e)
{
 ...
 * @brief 鼠标松开
 */
void TextButtonTab::mouseReleaseEvent(QMouseEvent *e)
{
//    clearFocus();
    if (e->button() == Qt::RightButton)
        return;
...
void TextButtonTab::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
...
void TextButtonTab::focusOutEvent(QFocusEvent *e)
{
    emit updateInterface();
    QPushButton::focusOutEvent(e);
}

void TextButtonTab::keyPressEvent(QKeyEvent *e)
{
    switch (e->key()) {
    case Qt::Key_Up:
        emit focus(0);
