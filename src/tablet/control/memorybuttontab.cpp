...
MemoryButtonTab::MemoryButtonTab(const QString &text, bool listwidgetbtn, QWidget *parent)
    : TextButtonTab(text, parent)
    , m_effect(new QGraphicsDropShadowEffect(this))
    , m_isallgray(false)
{
    m_settings = DSettingsAlt::instance(this);
    setObjectName("MemoryButtonTab");
...
MemoryButtonTab::~MemoryButtonTab()
{
    // delete m_effect;
}

void MemoryButtonTab::init()
{
 ...
 * @param msec 100ms
 */
void MemoryButtonTab::animate(bool isspace, int msec)
{
 ...
 * @brief 切换M˅，
 */
void MemoryButtonTab::setbtnlight(bool light)
{
 ...
 * 历史区域显示，除mlist外按钮置灰
 */
void MemoryButtonTab:: setbuttongray(bool memorywidgetshow)
{
    //20200701 精简代码
 ...
 */
void MemoryButtonTab::updateWhenBtnDisable()
{
 ...
 * @brief 鼠标按下
 */
void MemoryButtonTab::mousePressEvent(QMouseEvent *e)
{
 ...
 * @brief 鼠标松开
 */
void MemoryButtonTab::mouseReleaseEvent(QMouseEvent *e)
{
    if (e->button() == Qt::RightButton)
        return;
    this->setPalette(m_palette);
...
void MemoryButtonTab::enterEvent(QEvent *e)
{
...
void MemoryButtonTab::leaveEvent(QEvent *e)
{
    m_font.setPointSize(16);
    m_font.setStyleName("Light");
    m_isHover = false;
...
void MemoryButtonTab::paintEvent(QPaintEvent *e)
{
    Q_UNUSED(e);
    if (isEnabled() == false) {
        m_font.setPointSize(16);
    ...
    if (hasFocus()) {
        ...
        if (m_isPress) {
            ...
            if (this->text() == "MH˅" || this->text() == "MH˄") {
                this->paintTextNormal(painter, normal);
            } else
        ...
        if (m_isHover) { //hover状态
            ...
            painter.setFont(m_font);
            if (this->text() == "MH˅" || this->text() == "MH˄") {
                this->paintTextHover(painter, normal);
            } else
                painter.drawText(textRect, this->text());
...
void MemoryButtonTab::focusOutEvent(QFocusEvent *e)
{
    emit updateInterface();
    QPushButton::focusOutEvent(e);
}

void MemoryButtonTab::paintTextNormal(QPainter &painter, QRectF normal)
{
    if (this->text() == "MH˅") {
        QRectF textRect1 = QRectF(normal.left() + 57, normal.top() + 62, 10, 17);
        QFont a1;
        a1.setPointSize(12);
        a1.setFamily("Noto Sans");
        painter.setFont(a1);
        painter.drawText(textRect1, "M");
        QRectF textRect2 = QRectF(normal.left() + 64, normal.top() + 67, 10, 17);
        painter.drawLine(textRect2.bottomLeft(), textRect2.topRight());
        QRectF textRect3 = QRectF(normal.left() + 72, normal.top() + 69, 9, 17);
        QFont a3;
        a3.setPointSize(12);
        a3.setFamily("Noto Sans");
        painter.setFont(a3);
        painter.drawText(textRect3, "H");
        QRectF textRect4 = QRectF(normal.left() + 80, normal.top() + 60, 9, 17);
        QFont a4;
        a4.setPointSize(20);
...
void MemoryButtonTab::paintTextHover(QPainter &painter, QRectF normal)
{
    if (this->text() == "MH˅") {
        QRectF textRect1 = QRectF(normal.left() + 57, normal.top() + 62, 12, 17);
        QFont a1;
        a1.setPointSize(13);
        a1.setFamily("Noto Sans");
        painter.setFont(a1);
        painter.drawText(textRect1, "M");
        QRectF textRect2 = QRectF(normal.left() + 65, normal.top() + 67, 10, 17);
        painter.drawLine(textRect2.bottomLeft(), textRect2.topRight());
        QRectF textRect3 = QRectF(normal.left() + 72, normal.top() + 69, 9, 17);
        QFont a3;
        a3.setPointSize(13);
