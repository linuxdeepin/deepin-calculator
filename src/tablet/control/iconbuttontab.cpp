...
const QSize HISTORY_WIDGET_CLEARBUTTONSIZE = QSize(52, 51); //历史记录区垃圾桶大小
const qreal BLURRADIUS = 12; //阴影模糊半径
const qreal ROUND_XRADIUS = 8; //按钮圆角x轴半径
const qreal ROUND_YRADIUS = 8; //按钮圆角y轴半径

IconButtonTab::IconButtonTab(QWidget *parent, int b, bool page)
    : TextButtonTab("", parent)
 ...
 */
void IconButtonTab::setIconUrl(const QString &normalFileName, const QString &hoverFileName, const QString &pressFileName, int mode)
{
...
/**
 * @brief IconButtonTab物理键盘点击动画效果
 * @param msec 100ms
 */
void IconButtonTab::animate(bool isspace, int msec)
{
 ...
 */
void IconButtonTab::updateWhenBtnDisable()
{
    m_isPress = false;
    m_buttonStatus = 0;
    m_mode = 1;
 ...
 * @brief 点击时改变标置位
 */
void IconButtonTab::mousePressEvent(QMouseEvent *e)
{
    m_currentUrl = m_pressUrl;
    m_buttonStatus = 2;
    if (m_mode == 1)
    ...
    TextButtonTab::mousePressEvent(e);
}

/**
 * @brief 鼠标松开时改变标志位
 */
void IconButtonTab::mouseReleaseEvent(QMouseEvent *e)
{
//    if (m_isHistorybtn)
//        clearFocus();
    m_currentUrl = m_normalUrl;
...
//            PerformanceMonitor::startOperate();
    ...
    TextButtonTab::mouseReleaseEvent(e);
}

void IconButtonTab::enterEvent(QEvent *e)
{
    m_currentUrl = m_hoverUrl;
    m_buttonStatus = 1;
    m_isHover = true;
    ...
    TextButtonTab::enterEvent(e);
}

void IconButtonTab::leaveEvent(QEvent *e)
{
    m_currentUrl = m_normalUrl;
    m_buttonStatus = 0;
    m_isHover = false;
    ...
    TextButtonTab::leaveEvent(e);
}

void IconButtonTab::paintEvent(QPaintEvent *)
{
...
//                    }
        ...
        if (type == 1) {
            pressBrush = QColor(230, 230, 230);
//            pressBrush.setAlphaF(0.7);
            focus = actcolor;
            base = Qt::transparent;
            hoverbrush = QColor(230, 230, 230);
//            hoverbrush.setAlphaF(0.1);
        } else {
            pressBrush = QColor(45, 45, 45);
//            pressBrush.setAlphaF(0.2);
            focus = actcolor;
            base = Qt::transparent;
            hoverbrush = pressBrush;
        }
        if (hasFocus()) {
            if (m_isPress) {
                painter.setBrush(QBrush(pressBrush));
...
void IconButtonTab::keyPressEvent(QKeyEvent *e)
{
    switch (e->key()) {
    case Qt::Key_Up:
        emit focus(0);
...
/**
 * @brief 更改IconButtonTab图片为活动色，且将图片移至IconButtonTab中间
 */
void IconButtonTab::drawCenterPixMap(QPainter &painter)
{
    painter.save();
...
/*void IconButtonTab::setIconSize(const int &size)
{
