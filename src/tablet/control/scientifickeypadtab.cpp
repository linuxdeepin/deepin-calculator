...
const int KEYPADHEIGHT = 714; //整体键盘的高度
const int KEYPADHEIGHTVER = 1165; //整体键盘的高度
const QSize BUTTON_SIZE = QSize(231, 80); //科学模式的固定大小
const QSize BUTTONVER_SIZE = QSize(167, 157); //科学模式的固定大小

const ScientificKeyPadTab::KeyDescription ScientificKeyPadTab::keyDescriptions[] = {
 ...
 */
//static DPushButton *createSpecialKeyButton(ScientificKeyPadTab::Buttons key, bool page)
//{
//    IconButton *button;
//    if (page)
    ...
    , m_exwidget(new QStackedWidget(this))
{
    ...
    m_gridlayout1->setSpacing(8); //按钮比ui大2pix,此处小2pix
    ...
    connect(this, &ScientificKeyPadTab::buttonPressed, this,
            &ScientificKeyPadTab::turnPage); //按下2nd事件
    connect(this, &ScientificKeyPadTab::buttonPressedbySpace, this,
            &ScientificKeyPadTab::turnPage);
}

ScientificKeyPadTab::~ScientificKeyPadTab()
{
}

void ScientificKeyPadTab::mouseMoveEvent(QMouseEvent *e)
{
    Q_UNUSED(e);
}
 ...
 * @param key 按钮名
 */
DPushButton *ScientificKeyPadTab::button(Buttons key)
{
...
DPushButton *ScientificKeyPadTab::button(int key)
{
    if (key == Key_arcsin || key == Key_arccos || key == Key_arccot ||
            key == Key_arctan || key == Key_sqrt2 || key == Key_sqrt3 ||
            key == Key_ysqrtx || key == Key_2x || key == Key_logyx || key == Key_ex) {
 ...
 * @brief 按钮点击动画效果
 */
void ScientificKeyPadTab::animate(Buttons key, bool isspace)
{
...
bool ScientificKeyPadTab::buttonHasFocus()
{
 ...
 * @brief 初始化button
 */
void ScientificKeyPadTab::initButtons()
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (desc->text.isEmpty()) {
            if (i > 5 && (i % 6 == 0 || i % 6 == 1))
                button = new IconButtonTab(this, 0, true);
            else
                button = new IconButtonTab(this, 0, false);
        } else {
            if (desc->text == "=") {
                button = new EqualButtonTab(desc->text, this);
                connect(static_cast<EqualButtonTab *>(button), &EqualButtonTab::focus, this, &ScientificKeyPadTab::getFocus); //获取上下左右键
                ...
                if (i > 5 && (i % 6 == 0 || i % 6 == 1))
                    button = new TextButtonTab(desc->text, true, this);
                else {
        ...
        if ((i / 6 > 1) && (i % 6 == 0 || i % 6 == 1) && i != 36 && i != 42) {
            ...
            if (desc->button == Key_sin) {
                ...
                pagebutton = new IconButtonTab(this, 0, true);
                ...
                pagebutton = new TextButtonTab("arccos", true, this);
                ...
                pagebutton = new TextButtonTab("arctan", true, this);
                ...
                pagebutton = new TextButtonTab("arccot", true, this);
                ...
                pagebutton = new TextButtonTab("2ˣ", true, this);
        ...
        connect(static_cast<TextButtonTab *>(button), &TextButtonTab::focus, this, &ScientificKeyPadTab::getFocus); //获取上下左右键
        ...
        connect(static_cast<TextButtonTab *>(button), &TextButtonTab::moveLeft, this, &ScientificKeyPadTab::moveLeft);
        connect(static_cast<TextButtonTab *>(button), &TextButtonTab::moveRight, this, &ScientificKeyPadTab::moveRight);
 ...
 */
void ScientificKeyPadTab::initUI()
{
    QHashIterator<Buttons, QPair<DPushButton *, const KeyDescription *>> i(m_keys);
    ...
    while (i.hasNext()) {
        ...
        if (i.key() == Key_left) {
            ...
            connect(static_cast<TextButtonTab *>(i.value().first), &TextButtonTab::mouseRelease, [ = ]() {
    ...
    this->setContentsMargins(20, 0, 19, 20);
}

/**
 * @brief ScientificKeyPadTab::initStackWidget
 * @param widget
 ...
 * page2按键初始化设置
 */
void ScientificKeyPadTab::initStackWidget(QStackedWidget *widget, DPushButton *button, DPushButton *pagebutton, const KeyDescription1 *desc1)
{
    ...
    connect(static_cast<TextButtonTab *>(pagebutton), &TextButtonTab::moveLeft, this, &ScientificKeyPadTab::moveLeft);
    connect(static_cast<TextButtonTab *>(pagebutton), &TextButtonTab::moveRight, this, &ScientificKeyPadTab::moveRight);
    connect(static_cast<TextButtonTab *>(pagebutton), &TextButtonTab::space, this, [ = ]() {
 ...
 * @brief 切换相应主题切图
 */
void ScientificKeyPadTab::buttonThemeChanged(int type)
{
    m_themetype = type;
    QString path;
    if (type == 2) {
    ...
    btn = static_cast<IconButtonTab *>(button(Key_ysqrtx));
 ...
 */
void ScientificKeyPadTab::turnPage(int key)
{
    if (key == Key_page) {
...
/**
 * @brief ScientificKeyPadTab::getdeg
 * @param deg 当前为deg/rad/grad
 * 切换deg按键图标
 */
void ScientificKeyPadTab::getdeg(int deg)
{
    m_deg = deg;
    QString path;
    if (m_themetype == 2)
...
/**
 * @brief ScientificKeyPadTab::bracketsNum
 * @param direction 0-左括号， 1-右括号
 * @param num 不匹配括号数
 * 显示左右括号不匹配数
 */
void ScientificKeyPadTab::bracketsNum(int direction, QString num)
{
    if (num == "0")
        num = "";
 ...
 */
void ScientificKeyPadTab::getFocus(int direction)
{
    QHashIterator<Buttons, QPair<DPushButton *, const KeyDescription *>> i(m_keys);
