...
ExpressionBarTab::ExpressionBarTab(QWidget *parent)
    : DWidget(parent)
{
    ...
    m_inputEdit->setTextMargins(19, 0, 19, 6);
//    m_inputEdit->setFocus();

    m_listView->setModel(m_listModel);
    m_listView->setItemDelegate(m_listDelegate);
    m_listView->setFixedHeight(204);
...
ExpressionBarTab::~ExpressionBarTab() {}

void ExpressionBarTab::mouseMoveEvent(QMouseEvent *e)
{
    Q_UNUSED(e);
}
 ...
 * 对应的数字
 */
void ExpressionBarTab::enterNumberEvent(const QString &text)
{
    //    if (m_isLinked)
    //        clearLinkageCache();
 ...
 * 对应的运算符
 */
void ExpressionBarTab::enterSymbolEvent(const QString &text)
{
    QString symbol = text;
 ...
 */
void ExpressionBarTab::enterPercentEventCommon()
{
//    if (m_inputEdit->text().isEmpty()) {
//        m_inputEdit->setText("0");
//        return;
 ...
 * 输入小数点
 */
void ExpressionBarTab::enterPointEvent()
{
    //    if (m_isLinked)
    //        clearLinkageCache();
    replaceSelection(m_inputEdit->text());
 ...
 * 输入退格键
 */
void ExpressionBarTab::enterBackspaceEvent()
{
 ...
 * C/AC的清空事件
 */
void ExpressionBarTab::enterClearEvent()
{
    if (m_isAllClear) {
 ...
 */
void ExpressionBarTab::enterEqualEvent()
{
    QString oldtext = m_inputEdit->text();
    if (m_inputEdit->text().isEmpty())
        return;
 ...
 * 当前使用的输入百分号的事件
 */
void ExpressionBarTab::enterPercentEvent()
{
    if (m_inputEdit->text().isEmpty()) {
//        m_inputEdit->setText("0%");
        return;
 ...
 */
void ExpressionBarTab::enterBracketsEvent()
{
    if (!m_hisLink.isEmpty() && m_hisLink.last().linkedItem == -1) {
        m_hisLink.last().linkedItem = m_listModel->rowCount(QModelIndex());
        m_hisLink.last().isLink = true;
 ...
 */
void ExpressionBarTab::enterLeftBracketsEvent()
{
    if (!m_hisLink.isEmpty() && m_hisLink.last().linkedItem == -1) {
        m_hisLink.last().linkedItem = m_listModel->rowCount(QModelIndex());
        m_hisLink.last().isLink = true;
...
void ExpressionBarTab::enterRightBracketsEvent()
{
    if (!m_hisLink.isEmpty() && m_hisLink.last().linkedItem == -1) {
        m_hisLink.last().linkedItem = m_listModel->rowCount(QModelIndex());
        m_hisLink.last().isLink = true;
 ...
 * 将输入栏中的内容复制到剪贴板
 */
void ExpressionBarTab::copyResultToClipboard()
{
 ...
 * 将剪贴板的内容复制到输入栏
 */
void ExpressionBarTab::copyClipboard2Result()
{
    if (!m_hisLink.isEmpty() && m_hisLink.last().linkedItem == -1) {
        m_hisLink.last().linkedItem = m_listModel->rowCount(QModelIndex());
        m_hisLink.last().isLink = true;
 ...
 * 全选事件
 */
void ExpressionBarTab::allElection()
{
 ...
 * @brief 删除事件，选中删除
 */
void ExpressionBarTab::deleteText()
{
    QString text = m_inputEdit->text();
    int selcurPos = m_inputEdit->cursorPosition();
    int start = m_inputEdit->selectionStart();
 ...
 * @param text
 */
void ExpressionBarTab::handleTextChanged(const QString &text)
{
    Q_UNUSED(text);
    m_isAllClear = false;
    m_isContinue = true;
...
bool ExpressionBarTab::cursorPosAtEnd()
{
 ...
 * 替换后的表达式
 */
QString ExpressionBarTab::formatExpression(const QString &text)
{
    return QString(text)
           .replace(QString::fromUtf8("＋"), "+")
           .replace(QString::fromUtf8("－"), "-")
 ...
 * 点击的行号/索引
 */
void ExpressionBarTab::revisionResults(const QModelIndex &index)
{
 ...
 */
void ExpressionBarTab::clearLinkageCache(const QString &text, bool isequal)
{
 ...
 */
void ExpressionBarTab::settingLinkage()
{
//    m_evaluator->unsetVariable(QLatin1String("e"), true);
    ...
    if (hisRecision != -1) {
        ...
        historicalLinkageIndexTab hisIndex;
...
void ExpressionBarTab::initConnect()
{
    ...
    connect(m_inputEdit, &InputEdit::equal, this, &ExpressionBarTab::enterEqualEvent);
    connect(m_inputEdit, &InputEdit::cut, this, &ExpressionBarTab::shear);
    connect(m_inputEdit, &InputEdit::copy, this, &ExpressionBarTab::copyResultToClipboard);
    connect(m_inputEdit, &InputEdit::paste, this, &ExpressionBarTab::copyClipboard2Result);
    connect(m_inputEdit, &InputEdit::deleteText, this, &ExpressionBarTab::deleteText);
    connect(m_inputEdit, &InputEdit::selectAllText, this, &ExpressionBarTab::allElection);
    connect(m_inputEdit, &InputEdit::undo, this, &ExpressionBarTab::Undo);
    connect(m_inputEdit, &InputEdit::redo, this, &ExpressionBarTab::Redo);
    connect(m_inputEdit, &InputEdit::setResult, this, &ExpressionBarTab::setResultFalse);
}
 ...
 * @return
 */
QString ExpressionBarTab::symbolComplement(const QString exp)
{
    QString text = exp;
 ...
 * @return
 */
QString ExpressionBarTab::pointFaultTolerance(const QString &text)
{
...
void ExpressionBarTab::expressionCheck()
{
    QString exp = m_inputEdit->text();
    int cur = m_inputEdit->cursorPosition();
 ...
 * 主题类型
 */
void ExpressionBarTab::initTheme(int type)
{
    //edit for bug-21476
    int typeIn = type;
    if (typeIn == 0) {
...
void ExpressionBarTab::setResultFalse()
{
    m_isResult = false;
}
 ...
 * @param text
 */
void ExpressionBarTab::replaceSelection(QString text)
{
    QString seloldtext = text;
    SSelection selection = m_inputEdit->getSelection();
    int selcurPos = m_inputEdit->cursorPosition();
...
bool ExpressionBarTab::cancelLink(int index)
{
    bool isRemove = false;
...
bool ExpressionBarTab::isOperator(const QString &text)
{
...
void ExpressionBarTab::moveLeft()
{
    m_inputEdit->setCursorPosition(m_inputEdit->cursorPosition() - 1);
    m_inputEdit->setFocus();
}

void ExpressionBarTab::moveRight()
{
    m_inputEdit->setCursorPosition(m_inputEdit->cursorPosition() + 1);
    m_inputEdit->setFocus();
}
