    ...
    : DWidget(parent)
{
    ...
    connect(m_scihiswidget->findChild<MemoryWidgetTab *>(), &MemoryWidgetTab::mListUnavailable, this, &scientificModuleTab::mUnAvailableEvent);
    connect(m_scihiswidget->findChild<MemoryWidgetTab *>(), &MemoryWidgetTab::mListAvailable, this, &scientificModuleTab::mAvailableEvent);
...
{
    TextButtonTab *btn = static_cast<TextButtonTab *>(m_scikeypadwidget->button(ScientificKeyPadTab::Key_FE));
    if (isdown) {
        m_FEisdown = true;
    } else {
        m_FEisdown = false;
