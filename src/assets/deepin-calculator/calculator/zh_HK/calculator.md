# 計數機|../common/deepin_calculator_96.svg|
...
| +_×÷                       | 加减乘除 | 基本數學運算符，用來進行加法、減法、乘法、除法運算。         |
| .                          | 小數點   | 用來輸入小數點。                                             |
...
| 2<sup>nd</sup> | 第二功能鍵 | 單擊 **2<sup>nd</sup>** 切換到反三角函數和開根運算界面，再次單擊切換到三角函數與次方運算界面。 |
...
- 減法符號容錯處理：輸入 _ (減號）或_ (下劃線）都會觸發減法符號啟動；
...
- 當計算結果為小數且大於16位/32位時，顯示數字+小數點後15位/31位+E_數字。
