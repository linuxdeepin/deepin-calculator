# 计算器|../common/deepin_calculator_96.svg|
...
- 选择 **标准模式**：执行基本的加减乘除运算。
- 选择 **科学模式**：执行函数、指数、方根等高级运算。
- 选择 **程序员模式**：执行二进制、八进制、十进制、十六进制等复杂运算。
...
|  ![icon](../common/M.svg) / ![icon](../common/M^.svg)            | 存储键  | 点击![icon](../common/M.svg)展开内存列表，点击![icon](../common/M^.svg)折叠内存列表；关闭应用后内存清零。 |
